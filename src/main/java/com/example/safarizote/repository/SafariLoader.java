...
public class SafariLoader implements CommandLineRunner {
    ...
    public List<Booking> createBookings() {
        /*
         * List<Booking> bookings = new ArrayList<>(); List<Safari> safaris =
         * repository.findAll(); Safari[] myArray = new Safari[safaris.size()];
         * safaris.toArray(myArray); Set<Safari> safarisSet = new HashSet<>(safaris);
         * int id = (int) ((Math.random() * (myArray.length - 0)) + 0); Safari parent =
         * null;
         * System.out.println("\n==============> 2. New Enhanced For loop Example..");
         * for (Safari safari : safaris) { if (safari.getId() == id) {
         * System.out.println(parent); parent = safari; } }
         */
        ...
        bookingRepository.save(Booking.builder().name("Jack Maji Moto Smith").email("m@gmail.com").phone("415 22 386")
                .address("21 Jump street").adults("2").children("1").message("Twende Kenya ndugu yangu!!")
                .date(Instant.now()).build());
        bookingRepository.save(Booking.builder().name("Adam Moto Wake").email("maji@gmail.com").phone("915 22 111")
                .address("Grefsen Platåen").adults("2").children("1").message("Twende Mombasa tukaogelee!!")
                .date(Instant.now()).build());
        bookingRepository.save(Booking.builder().name("Johnson Katana Ndovu").email("moto@hotmail.com")
                .phone("222 22 222").address("Maridalsveien").adults("2").children("1")
                .message("Twende Safari tukaone wanyama!!").date(Instant.now()).build());
        bookingRepository.save(Booking.builder().name("Peter Ngara Mwendwa").email("kazi@online.no").phone("911 22 911")
                .address("Number 10").adults("2").children("1").message("Twende Safari tukaone Ndovu!!")
                .date(Instant.now()).build());
        bookingRepository.save(Booking.builder().name("Masinde Murilo David").email("sverige@kora.se")
                .phone("+44 510 22 777").address("Downings Street").adults("2").children("1")
                .message("Twende Safari tukaone Simba na wenzake!!").date(Instant.now()).build());
        ...
        /*
         * Set<Booking> hSet = new HashSet<Booking>(bookings); Booking b =
         * hSet.stream().findFirst().get(); System.out.println(b); safaris =
         * repository.findAll(); for (int i = 0; i < safaris.size(); i++) { Safari
         * safari = safaris.get(i); safari.setBooking(b); repository.save(safari); }
         */
        // List<Booking> bookings = bookingRepository.findAll();
